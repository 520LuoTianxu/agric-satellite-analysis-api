...
- Vegetation pipeline (classic): STAC search → download bands → compute index → write COG → zonal stats → alerts. Agri-tagged fields: STAC bands + optional SCL → compute optical indices in memory → `lonlat_v1` upsert (`parcel_scene_products`); `parcel_cloud_cover_pct` is in-polygon SCL/clear cloud, not zonal quality_score; no index TIF unless `WRITE_INDEX_COGS=1`. Optional `DECLOUD_ENABLED` parcel-window UnCRtainTS decloud is additive (STAC search default 90%); only quality `good` is official.
