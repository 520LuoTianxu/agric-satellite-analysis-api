...
- Shared factory in `packages/openfarm_common/openfarm_common/celery_app.py` (broker, routes, Redis transport). API/ingest/storage call `create_celery_app`.
...
- Vegetation pipeline (classic): STAC search → download bands → compute index → write COG → zonal stats → alerts. Agri-tagged fields: STAC bands + optional SCL → compute optical indices in memory → `lonlat_v1` upsert (`parcel_scene_products`); `parcel_cloud_cover_pct` is in-polygon SCL/clear cloud, not zonal quality_score (nodata/out-of-range SCL and missing SCL store NULL, not 0); no index TIF unless `WRITE_INDEX_COGS=1`. Optional `DECLOUD_ENABLED` parcel-window UnCRtainTS decloud is additive (default `DECLOUD_MODE=batch` after buffering neighbors; STAC search default 90%); `good`/`fair`/`bad` are stored, only quality `good` is official for drought, NDVI growth pick may prefer physiological raw. Tooltips fall back to STAC when stored parcel cloud is ~0% while scene cloud is 80%+.
