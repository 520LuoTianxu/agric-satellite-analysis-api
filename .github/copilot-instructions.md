...
- Vegetation pipeline (classic): STAC search → download bands → compute index → write COG → zonal stats → alerts. Agri-tagged fields: STAC bands → compute optical indices in memory → `lonlat_v1` upsert (`parcel_scene_products`); no index TIF unless `WRITE_INDEX_COGS=1`. Optional `DECLOUD_ENABLED` parcel-window UnCRtainTS decloud is additive; only quality `good` is official.
