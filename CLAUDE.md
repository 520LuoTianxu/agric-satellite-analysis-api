...
- **Satellite pipeline:** ingest `app.tasks.pipeline` (classic: STAC search → band reads → index compute → COG write → zonal stats → alerts). Agri-tagged fields use `app.tasks.agri_lonlat`: same STAC bands plus optional SCL, compute NDVI/EVI/NDMI/NDRE/CIre/MNDWI in memory, sample `lonlat_v1`, upsert `agri.parcel_scene_products` (no index TIF/COG unless `WRITE_INDEX_COGS=1`). `parcel_cloud_cover_pct` is in-polygon SCL / lonlat-clear cloud, not zonal `quality_score`. Optional `DECLOUD_ENABLED` UnCRtainTS parcel-window decloud writes an additive product; only quality `good` enters official drought metrics (see `docs/decloud-uncrtaints.md`). Index definitions in ingest `tasks/indices.py`. Backfill = 60 months in 90-day chunks (`tasks/backfill.py`). MAX_CLOUD_COVER=20 module constant (raised to `DECLOUD_STAC_CLOUD_MAX_PCT`, default 90, when decloud is on).
