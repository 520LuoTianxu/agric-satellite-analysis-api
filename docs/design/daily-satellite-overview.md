...
每日批处理先将有效地块匹配到已有虚拟项目区，未匹配地块由10×10公里全局锚点贪心规划器分组。任务以虚拟项目区为下载和缓存单位：下载机先复用OSS中的区域像素资产，没有缓存时才检索并下载整区；之后按每块地的完整边界裁剪，把S2指数和S1的VV/VH结果写回原地块。更新后的Smart边界仍须完整落在原项目区内，否则解除旧归属并规划新区域。
