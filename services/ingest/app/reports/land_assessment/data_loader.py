...
def load_indices_from_field_stats(session: Session, land_id: str) -> list[dict]:
    rows = session.execute(
        ...
        .where(FieldStat.land_id == str(land_id))
...
def load_soil(session: Session, land_id: str) -> dict[str, Any]:
    s = session.execute(
        select(SoilFieldSummary).where(SoilFieldSummary.land_id == str(land_id))
    ).scalar_one_or_none()
    out: dict[str, Any] = {}
    if s:
        out = {
    ...
    if SoilNutrientNpk is not None:
        npk = session.execute(
            select(SoilNutrientNpk).where(SoilNutrientNpk.land_id == str(land_id))
        ).scalar_one_or_none()
        if npk:
            out["npk"] = {
                "source": npk.source,
...
def load_weather(session: Session, land_id: str) -> tuple[dict, dict]:
    ...
    rows = (
        session.execute(
            select(WeatherDaily)
            .where(
                WeatherDaily.land_id == str(land_id),
...
def load_suitability_sync(
    ...
    profile = session.execute(
        select(SoilProfile).where(SoilProfile.land_id == str(land_id))
    ).scalar_one_or_none()
...
def load_site_admission(
    ...
    row = session.execute(
        select(GroupSiteAdmission).where(GroupSiteAdmission.land_id == str(land_id))
    ).scalar_one_or_none()
    if row is None:
        return None
...
def load_land_bundle(
    ...
    *,
    ...
    """Load everything needed to score + render one canonical land parcel.

    When ``API_BASE_URL`` + ``INTERNAL_API_TOKEN`` are set (download host), prefer
    ``GET /v1/internal/lands/{land_id}/assessment-bundle``. SyncSession is used only
    ...
    return {
        "field": {
            "id": land_id,
            "name": land.land_name or land_id,
            ...
            "crop_key": crop_key,
            "land_id": land_id,
            "boundary_source": "canonical_land_parcel",
        },
        "indices": indices,
...
def build_flood_evidence(
    ...
    *,
    ...
    if session is not None and land_id is not None:
        ...
        hi = max(dates)
        series = load_daily_precipitation(session, str(land_id), lo, hi)
        for s in selected:
