"""Celery Beat 入口：请求 API 机按当前地块清单调度历史遥感回填。"""
...
@celery_app.task(
    name="app.tasks.satellite_history.schedule_satellite_history_backfill",
    bind=True,
    max_retries=3,
    time_limit=300,
    soft_time_limit=240,
...
def schedule_satellite_history_backfill(
    self,
    *,
    as_of: str | None = None,
    admin_task_run_id: str | None = None,
) -> dict[str, Any]:
    """仅 API 机访问 PostgreSQL 并规划；下载机不连接业务数据库。"""
    if not internal_api_enabled():
        raise RuntimeError("历史遥感回填需要 API_BASE_URL 和 INTERNAL_API_TOKEN")
    try:
        result = satellite_history_backfill(as_of=as_of)
    except Exception as exc:
        logger.exception("satellite_history_http_failed", as_of=as_of)
