...
async def _get_weather_snapshot(db: AsyncSession, land_id: str) -> dict | None:
...
@router.get(
    "/lands/{land_id}/scouting", response_model=PaginatedResponse[ScoutingOut]
)
async def list_scouting(
    ...
    base = select(ScoutingObservation).where(
        ScoutingObservation.land_id == land_id, org_scope(None, ctx)
    )
    total = (
...
async def create_scouting(
    ...
    weather_snapshot = await _get_weather_snapshot(db, land_id)
    ...
    db.add(obs)
    await db.flush()
    logger.info("scouting_created", obs_id=str(obs.id), land_id=str(land_id))
    return _obs_to_out(obs)


@router.patch("/lands/{land_id}/scouting/{obs_id}", response_model=ScoutingOut)
async def update_scouting(
    ...
    if not obs or obs.land_id != land_id:
...
@router.delete(
    "/lands/{land_id}/scouting/{obs_id}", status_code=status.HTTP_204_NO_CONTENT
)
