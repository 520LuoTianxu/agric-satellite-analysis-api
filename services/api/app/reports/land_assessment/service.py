# -*- coding: utf-8 -*-
"""Orchestrate canonical land-assessment PDF generation."""
...
def generate_assessment_pdf(
    *,
    ...
    land_id = field.get("land_id") or field.get("id") or land_id
    ...
    return {
        ...
        "land_id": field.get("land_id") or field.get("id"),
        ...
        "ai_reference_disclaimer": (
            ai.get("ai_reference_disclaimer") if isinstance(ai, dict) else None
        ),
        "scorecard": computed["scorecard"],
        "rs": computed["rs"],
        "risk": computed["risk"],
