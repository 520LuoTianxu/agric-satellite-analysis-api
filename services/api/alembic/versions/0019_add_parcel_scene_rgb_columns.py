"""Add rgb preview columns on agric_satellite.parcel_scene_products.
...
def upgrade() -> None:
    ...
    op.execute(
        """
        COMMENT ON COLUMN agric_satellite.parcel_scene_products.rgb_url IS
          'Parcel true-color preview URL (OSS signed or public)'
        """
    )
    op.execute(
        """
        COMMENT ON COLUMN agric_satellite.parcel_scene_products.large_rgb_url IS
          'Optional larger true-color preview URL'
        """
    )
    op.execute(
        """
        COMMENT ON COLUMN agric_satellite.parcel_scene_products.rgb_oss_key IS
